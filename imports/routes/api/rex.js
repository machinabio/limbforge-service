...
Api.addRoute('rex/:hash', {
  post: function() {
    ...
    console.log(
      `-- Transaction ${id}: Receieved POST at /rex/${this.urlParams.hash}`,
    );
    //TODO add logging to analytics
    const script = Script.findOne({
      _md5: this.urlParams.hash,
      published: true,
    });
    if (script) {
      ...
      const query_handle = Transaction.find(transaction_id, {
      ...
      const watchog_handle = Meteor.setTimeout(() => {
        console.log(
          `-- Transaction ${id}: no response from Fusion360 agent before timeout`,
        );
        query_handle.stop();
      }, job_timeout);
      Fiber.yield();
