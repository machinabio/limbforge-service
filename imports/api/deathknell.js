...
var Deathknell = {
  ring: function() {
    console.log(
      'DEATHKNELL ring called after ' + (Date.now() - starttime) + ' ms',
    );
    Meteor.call(
      'printLog',
      'DEATHKNELL ring called after ' + String(Date.now() - starttime) + ' ms',
    );
  ...
  finish: function() {
    console.log(
      'DEATHKNELL finish called after ' + (Date.now() - starttime) + ' ms',
    );
    Meteor.call(
      'printLog',
      'DEATHKNELL finish called after ' +
  ...
  initialize: function() {
    console.log(
      'DEATHKNELL initialize called after ' + (Date.now() - starttime) + ' ms',
    );
    Meteor.call(
      'printLog',
      'DEATHKNELL initialize called after ' +
        String(Date.now() - starttime) +
        ' ms',
    );
...
function expireDeathknell() {
  ...
  console.error(
    'Shutting down after ' + (Date.now() - starttime) + ' milliseconds',
  );
  console.error('... after ' + rings + ' of ' + maxRings + ' rings');
  if (initialiaztionTO)
    console.error(
      '### Exceeded max initialization time of ' + maxInitTime + ' ms',
    );
  if (idleTO)
    console.error('### Exceeded max idle lifetime of ' + maxIdleTime + ' ms');
  if (executionTO)
    console.error('### Exceeded max execution time of ' + maxExecTime + ' ms');
  ...
  if (idleTO)
    Meteor.call(
      'printLog',
      '### Exceeded max idle lifetime of ' + maxIdleTime + ' ms',
    );
  if (executionTO)
    Meteor.call(
      'printLog',
      '### Exceeded max execution time of ' + maxExecTime + ' ms',
    );
  ...
  if (initialiaztionTO) {
    ...
    switch (expirationBehavior) {
      ...
      default:
        Meteor.call(
          'printLog',
          'expirationBehavior "' + expirationBehavior + '" is not recognized',
        );
...
function terminateMicroserver() {
  Meteor.call(
    'printLog',
    'Terminating Fusion360 agent down after ' +
      (Date.now() - starttime) +
      ' milliseconds',
  );
...
function reloadMicroserver() {
  Meteor.call(
    'printLog',
    'Reloading microserver after ' + (Date.now() - starttime) + ' milliseconds',
  );
